...
static void video_mux(struct TW68_dev *dev, int input)
{
	...
	printk("video %d input = %d [%s]\n", dev->ctl_input, input, card_in(dev, input).name);
	dev->ctl_input = input;
...
static int video_open(struct file *file)
{
	...
	printk(KERN_INFO " @@@@@ video_open   minor=%d  type=%s  openned:%X \n",
...
static int video_release(struct file *file)
{
   ...
   printk(KERN_INFO "video_release ()  CALLED  minor:%d  DMA_nCH: %X    video_fieldcount 0x%X ! \n",  minor, DMA_nCH, dev->video_fieldcount[DMA_nCH]);    /// 0-8  
	...
	else
	{
	 ...
	 printk(KERN_INFO " video_release  CALLED   !  dev->video_opened: 0x%x \n", dev->video_opened);
...
// pci_alloc_consistent   32 4 * 8  continuous field memory buffer
	...
	for (k=0; k<9; k++)
	{
		...
		/*
			...
			*/
			...
			printk("TW68_  _video_init1   get decoder %d default AMP: BRIGHTNESS %d  CONTRAST %d  HUE_ %d  SAT_U_%d SAT_V_%d   \n", k, 
				dev->video_param[k].ctl_bright, dev->video_param[k].ctl_contrast, dev->video_param[k].ctl_hue, dev->video_param[k].ctl_saturation, dev->video_param[k].ctl_mute);
	...
	// Normalize the reg value to standard value range
			for (k=0; k<9; k++)
			{
				...
				printk("TW68_  _video_init1   remap decoder %d default AMP: BRIGHTNESS %d  CONTRAST %d  HUE_ %d  SAT_U_%d SAT_V_%d   \n", k, 
				dev->video_param[k].ctl_bright, dev->video_param[k].ctl_contrast, dev->video_param[k].ctl_hue, dev->video_param[k].ctl_saturation, dev->video_param[k].ctl_mute);
...
int buffer_setup_QF(struct videobuf_queue *q, unsigned int *count, unsigned int *size)
{
	...
	*size = fh->fmt->depth * fh->width * fh->height >> 3;    // calculate byte size for 1 frame
	...
	for (nId = 0; nId < 4; nId++)
	{
		...
		{
			dwReg =  reg_readl(DECODER0_SDT+ (nId*0x10));
			printk(" ####%%%%%%%%%%   QF buffer_setup::  nId %d,  0X%X    nW%d nH%d nSize%d\n", nId, dwReg, nW, nH, nSize);
			reg_writel(DECODER0_SDT+ (nId*0x10), 7);		/// 0 NTSC
		}
		else
		{
	...
	printk(" $$$$$$$ QF  buffer_setup:: N%d   F%d  to %d  \n", nId, m_nVideoFormat, dev -> nVideoFormat[nId]);
			...
			printk(" #### QF  buffer_setup:: W:%d H:%d  frame size: %d,  gbuffers: %d, ChannelOffset: %d  field pgn: %d  m_StartIdx %d  m_EndIdx %d   m_nVideoFormat %x\n",
		...
		reg_writel( DMA_CH0_CONFIG+ nId, m_dwCHConfig);
		dwReg =  reg_readl(DMA_CH0_CONFIG+ nId);
		printk(" ********#### buffer_setup CH%d::  m_StartIdx 0X%x  pgn%d  dwReg: 0x%X   m_dwCHConfig 0x%X  \n", nId, m_StartIdx, pgn,  m_dwCHConfig, dwReg );
