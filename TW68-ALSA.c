...
static int snd_card_TW68_capture_trigger(struct snd_pcm_substream * substream, int cmd)
{
	...
	if (!card_TW68) {
		printk(KERN_ERR "BUG:  snd_card_TW68_capture_trigger TW68 can't find device struct.\n");
		return -ENODEV;
	}
...
static int snd_card_TW68_capture_prepare(struct snd_pcm_substream * substream)
{
	...
	if (!card_TW68) {
		printk(KERN_ERR "BUG:  snd_card_TW68_capture_prepare TW68 can't find device struct.\n");
		return -ENODEV;
	}
	dev = card_TW68 ->dev;
